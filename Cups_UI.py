...
class Cup:
    ...
    def draw_cup(self, x, y, width, height):
        radius = 0
        for i in range(self.capacity + 1):
            ...
            Rect.draw(Color[0], self.surface, 2, radius)    
        ...
        for i in range(len(self.list_Color)):
            ...
            self.list_Rect[i].draw(self.Color[color_array], self.surface, 2, radius)
        
        for i in range(len(self.list_Color), self.capacity):
            if i != 0:
                self.list_Rect[i].delete(Color[0], self.surface, 2, 0)
   
    def move_up(self, surface, x, y, w, h):
        self.surface.fill(Color[0])
        self.list_Rect = []
        self.draw_cup(10, 350, w, h)
...
class Cups:
    def __init__(self, inital_state, num_cups, capacity, surface, path, Color):
        ...
        self.width_per_surface = int(1500 / self.num_cups)
    ...
    def draw(self):
        for i in range (self.num_cups):
            self.list_Cups[i].draw_cup(10, 350, int(0.8 * self.width_per_surface), 50)
